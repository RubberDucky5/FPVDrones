...
const o_v1 = new Vector3();
const o_v2 = new Vector3();
...
class Octree {
  ...
  triangleCapsuleIntersect(capsule, triangle) {
    ...
    const intersectPoint = o_v1.copy(capsule.start).lerp(capsule.end, delta);
  ...
  triangleSphereIntersect(sphere, triangle) {
    ...
    const plainPoint = _plane.projectPoint(sphere.center, o_v1);
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      _line1.closestPointToPoint(plainPoint, true, o_v2);

      const d = o_v2.distanceToSquared(sphere.center);

      if (d < r2) {
        return {
          normal: sphere.center.clone().sub(o_v2).normalize(),
          point: o_v2.clone(),
  ...
  capsuleIntersect(capsule) {
    ...
    for (let i = 0; i < triangles.length; i++) {
      let result = this.triangleCapsuleIntersect(_capsule, triangles[i])
      if ((result)) {
        hit = true;
  ...
  fromGraphNode(group) {
    ...
    group.traverse((obj) => {
      ...
      // this makes everything better but makes "triggers" impossible
      //if((obj.userData.flag & 2) != 0) 
      //  return;
        
      // CMD.push(obj.userData.flag);
      if (obj.isMesh === true) {
        let geometry,
          isTemp = false;
